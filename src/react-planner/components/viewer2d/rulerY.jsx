...
export default class RulerY extends Component {
  ...
  render() {
    ...
    if (elementH <= 200) {
      for (let x = 1; x <= this.props.positiveUnitsNumber; x++) {
        positiveDomElements.push(
          <div key={x} style={{ ...elementStyle, gridColumn: 1, gridRow: x }}>
            {elementH > 30 ? ((this.props.positiveUnitsNumber - x) * 2) : ''}
          </div>
        );
      }
    }
      ...
      for (let x = 1; x <= this.props.positiveUnitsNumber; x++) {
        let val = (this.props.positiveUnitsNumber - x) * 24;
        positiveDomElements.push(
          <div key={x} style={{ ...elementStyle, gridColumn: 1, gridRow: x }}>
            <div style={insideElementsStyle}>{val + (3 * 6)}</div>
            <div style={insideElementsStyle}>{val + (2 * 6)}</div>
            <div style={insideElementsStyle}>{val + (1 * 6)}</div>
