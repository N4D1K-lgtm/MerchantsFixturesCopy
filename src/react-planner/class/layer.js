...
class Layer{
  ...
  static setPropertiesOnAllItemsInLayer(state, layerID, properties) {
    let items = state.getIn(['scene', 'layers', layerID, 'items']);
    items.forEach(itemID => state = Item.setProperties(state, layerID, itemID.id, properties).updatedState);

    return { updatedState: state }; // Add this return statement
  }
