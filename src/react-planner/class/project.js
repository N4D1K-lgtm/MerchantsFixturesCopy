...
class Project {
  ...
  static setGlobalTexture(state, textures) {
    // Map the plain object keys to the corresponding keys in the scene
    const textureKeyMap = {
      texture: "topTexture",
      texture1: "bodyTexture",
    };

    // Update the scene with the new textures
    let scene = state.get("scene");
    Object.entries(textures).forEach(([key, value]) => {
      const sceneKey = textureKeyMap[key];
      if (sceneKey) {
        scene = scene.set(sceneKey, value);
      }
    });

    // Define the property object based on the texture type
    const properties = fromJS(textures);
    // Update layers with the new properties
    state.getIn(["scene", "layers"]).forEach((layer) => {
      state = Layer.setPropertiesOnAllItemsInLayer(
        state,
        layer.get("id"),
        properties
      ).updatedState;
    ...
    // Merge the updated topTexture and bodyTexture properties into the state
    state = state.mergeIn(["scene"], {
      topTexture: scene.get("topTexture"),
      bodyTexture: scene.get("bodyTexture"),
    });
