...
import { UNIT_CENTIMETER, UNIT_MILLIMETER, UNIT_INCH, UNIT_FOOT } from '../constants';

export default class Catalog {

  constructor(unit = UNIT_FOOT) {
    this.elements = {};
    this.categories = {
      root: {
        name: 'root',
  ...
  */
  validateElement(json) {
    ...
    if (!json.info.hasOwnProperty('item_width')) throw new Error(`Element ${name} doesn't have item_width`);
    if (!json.info.hasOwnProperty('item_depth')) throw new Error(`Element ${name} doesn't have item_depth`);
  ...
  */
   ...
   *  @param {array} [children] Category's children
   *  @return {?object} Registered category
  */
  //   registerCategory(name, label, children) {
  registerCategory(name, label, type = 'child', description = '', parent = '', children = []) {
    if (this.validateCategory(name, label)) {
      this.categories[name] = { name, label, type, description, parent: parent, children: children, elements: [] };
      this.categories.root.categories.push(this.categories[name]);
      //console.log('child array', children);
      if (children && children.length) {
        children.forEach((childGroup) => {
          if (Array.isArray(childGroup)) {
            childGroup.forEach((el) => {
              if (!el.name) {
                console.error(`The element '${el}' is missing the 'name' property`);
                return;
              }
              this.addToCategory(name, el.name);
            });
          }
        });
      }
